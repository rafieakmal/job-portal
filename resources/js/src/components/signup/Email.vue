            ...
            <div class="row">
                ...
                <div class="col text-start text-md-start">
                    <h2 class="display-6 text-center text-md-start fw-semibold mb-5">
                        Enter the code generated on your email below to get
...
export default defineComponent({
    ...
    methods: {
        ...
        getCookie() {
            let cookie = this.$cookies.get('resend_timer_email');
            if (cookie != null) {
        ...
        async getCode() {
            try {
                const response = await axios.post('/api/candidate/get-code', {
                    token: this.token,
                    key: this.acceptKey,
                    receiver: this.$store.state.user.candidate_email,
                });
        ...
        startTimer() {
            this.interval = setInterval(() => {
                this.resendTimer--;
                this.$cookies.set('resend_timer_email', this.resendTimer, '1d');
                if (this.resendTimer === 0) {
                    clearInterval(this.interval);
                    this.resendCodeStatus = false;
                    this.$cookies.remove('resend_timer_email');
                }
            }, 1000);
        },
